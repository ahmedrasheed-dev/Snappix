...
const createPlaylist = asyncHandler(async (req, res) => {
  ...
  if (!name || !description) {
    throw new ApiError(400, "Name and description is required").send(res);
  }
  ...
  return new ApiResponse(200, playlist, "Playlist created successfully").send(
    res
  );
});
...
const getUserPlaylists = asyncHandler(async (req, res) => {
  ...
  return new ApiResponse(200, playlists, "Playlists fetched successfully").send(
    res
  );
});
...
const getPlaylistById = asyncHandler(async (req, res) => {
  ...
  if (!isValidObjectId(playlistId)) {
    throw new ApiError(400, "Playlist id is required and must be valid").send(
      res
    );
  }
  ...
  return new ApiResponse(200, playlist, "Playlist fetched successfully").send(
    res
  );
});
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
  ...
  if (playlist.videos.length === initialLength) {
    return new ApiResponse(200, playlist, "Video was not in the playlist").send(
      res
    );
  }

  await playlist.save({ validateBeforeSave: false });
  return new ApiResponse(200, playlist, "Video removed from playlist").send(
    res
  );
});
...
const deletePlaylist = asyncHandler(async (req, res) => {
  ...
  return new ApiResponse(200, playlist, "Playlist deleted successfully").send(
    res
  );
});
...
const updatePlaylist = asyncHandler(async (req, res) => {
  ...
  await playlist.save({ validateBeforeSave: false });
  return new ApiResponse(200, playlist, "Playlist updated successfully").send(
    res
  );
});
